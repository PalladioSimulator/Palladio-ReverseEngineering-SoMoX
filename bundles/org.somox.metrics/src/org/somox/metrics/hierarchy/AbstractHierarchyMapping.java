...
/**
 * Unified parent of package and directory mapping. Realizes hierarchy
 * conformance mapping metrics. Mapping metric: Checks how well component
 * candidates are arranged in the same or at least comparable packages /
 * directories. <br>
 * Naming: PackageDirectory must be read as an entity: can be either package or
 * directory at the instance level. TODO: check efficiency
 *
...
public abstract class AbstractHierarchyMapping<T extends Object> extends AbstractMetric { // SOMOXTODOCHANGE
    ...
    /**
     * Minimum likelihood of packages to be meaningfully considered somehow the same
     * package TODO: make configurable
     */
     ...
     * {@inheritDoc}
     *
     * Idea: 1) navigate to root for both elements 2) navigate down from root one
     * individual of the both elements deviates 3) continue navigation to concrete
     * element and memorize number of package steps. (optional) 4) map sum of
     * deviating steps to result (improve weights with this)
     *
     */
    @Override
    protected void internalComputeDirected(final ClusteringRelation relationToCompute) {
        ...
        if (maxHeight == 0) {
            ...
            relationToCompute.setResultMetric(getMID(), this.convertToNonLinearWeight(rawMeasure));
        }
    }

    /**
     * Return the path segment in the subclasses' meaning of hierarchy where this
     * GAST class is directly attached to. For example, for the directory hierachy,
     * return the directory in which this GAST class is located
     *
     * @param clazz The GASTClass for which to retrieve the path segment
     * @return The parent of the GASTClass as defined by our subclass or null if
     *         this class is not contained in the hierarchy as defined by the
     *         subclass
     */
     ...
     *
     * @param element The element (i.e. Directory or Package) for which to return
     *                the parent (i.e. parent package or parent directory)
    ...
    /**
     * Calculates the maximum common hierarchy levels of the trees given (starting
     * from root) For example, for a sets {a.b.c, a.b.d.e} and {a.b.g,a.b.h.z} the
     * maximum common path is a.b and hence the result would be 2.
     *
     * @param tree1 A prefix tree-based representation of all elements of the first
     *              set of paths
     * @param tree2 A prefix tree-based representation of all elements of the second
     *              set of paths
    ...
    private int getMaxCommonPackageDirectory(final TreeNode<T> tree1, final TreeNode<T> tree2) {
        ...
        // If there is a fork in one of the tree nodes, we consider them not to be the
        // same
        // as the route in the source tree becomes ambiguous. This might be subject to
        // change for
        ...
        if ((tree1.getChildren().size() > 1) || (tree2.getChildren().size() > 1)
                || (tree1.getElement() != tree2.getElement())) {
            return 0;
        }
    ...
    /**
     * Given a set of paths, we add all of them into a single prefix tree. This
     * allows efficient computation of the amount of similarity later in the
     * algorithm. If we first add a.b.c to the tree we get a->b->c. If we then add
     * a.b.c.d we get a->b->c->d. If we then add a.b.g we get a->b->{c->d | g}
     *
     * @param elements The set of GAST classes for which we build the prefix tree of
     *                 their hierarchy
     * @return The constructed tree
     */
    private TreeNode<T> collectPaths(final Set<ConcreteClassifier> elements) {
        final TreeNode<T> root = new TreeNode<>(); // create a new tree root
    ...
    /**
     * Add the given element to the given tree. The method first recursively moves
     * up in the list of path segments up until it reaches the root of the path.
     * Then it moves down the tree and for each path element it adds it to the tree.
     * If the node where the path segment should be added is not yet existing it
     * will be added to the tree.
     *
     * @param root    The tree to which we add the given elements path in the
     *                hierarchy
     * @param element The element for which we generate its path through the
     *                hierarchy which we then add to the tree
     * @return The last tree node in which we added the previous part of the
     *         element's path in the hierarchy
     */
    private TreeNode<T> addToTree(final TreeNode<T> root, final T element) {
        ...
        final TreeNode<T> newChild = new TreeNode<>(element);
